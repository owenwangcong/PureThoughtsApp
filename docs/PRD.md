# 善护念 PureThoughts · 产品需求文档 (PRD)

> 版本:**v0.6.1**(打坐計時支持**自訂任意时长**,见 §9.1;上一版 v0.6.0 **去群化:全 App 单一共修体,注册即入、无群 ID、无审核**,详见 §3,设计见 [`design/single-community.md`](design/single-community.md);v0.5.21 通知系统改造见 §5–§5.4) · 来源:`initial.md` + 十三轮需求澄清
> 技术栈:Flutter(iOS + Android) · **Supabase 自托管(Auth / Postgres+RLS / Edge Functions / pg_cron / Storage / Realtime)** · 推送(APNs + FCM)
>
> **v0.5 主要改动(相对 v0.4)**:
> 1. 决策落定:删号匿名化保留群总量 ✅ · 自定义功课项群成员均可加 ✅。
> 2. 六项建议功能转正:快捷报数、群公告、发愿随喜+回向文、连续用功天数(仅自己可见)、计数器整串提示、通知免打扰时段;群数据导出暂不做。
> 3. **Supabase 改为自托管**:新增 §12.5 部署要点(机房选址、备份、SMTP、Edge Functions);相应调整 NFR 备份项与大陆可达性风险。
> 4. **v0.5.1**:代报对象不限群成员;自由名字**自动记忆进本群代报名单**,下次输入可点选 / 自动补全(§4.2,新增 `proxy_names` 表)。

---

## 1. 产品概述

**善护念 / PureThoughts** 是面向共修团体的功课记录与学修一体化 App。核心是**每日功课报数与统计**(v0.6.0 起为**单一共修体**,不再有"群"的概念),并整合通知活动、直播回看、在线经本、念诵导引音视频、以及打坐工具。

- **目标用户**:全球华人共修社群成员,**约 1/3 在中国大陆**;**年龄跨度大**(需大字体、简洁交互)。
- **平台**:iOS + Android(Flutter 一套代码)。
- **语言**:简体 / 繁体,用户可切换,**默认繁体**。
- **文化基调**:随喜、不攀比 → **不做任何成员间排名**;个人统计仅自己可见,对外只见共修总量。

---

## 2. 角色与权限矩阵

> **v0.6.0 去群化**:角色只剩三档,「群主」整列取消,其能力并入 App 管理员;「加入报数群 / 审核入群 / 建群 / 转让 / 解散 / 重置群 ID」六行删除。

| 能力 | 匿名用户 | 注册用户 | App 管理员 |
|------|:---:|:---:|:---:|
| 浏览经本 / 通知 / 视频 / 日历 / 工具 | ✅ | ✅ | ✅ |
| **报数 / 代报 / 补报** | ❌(跳登录) | ✅ **注册即可,无需申请与审核** | ✅ |
| 修改 / 删除**自己提交的**报数 | ❌ | ✅ | ✅ |
| 删除任意报数记录 | ❌ | ❌ | ✅ |
| 查看个人统计 / 设发愿目标 | ❌ | ✅(仅自己可见) | ✅ |
| 查看共修总量与共修报数记录 | ❌ | ✅ | ✅ |
| 添加自定义功课项 | ❌ | ✅(**仅自己可见可选**) | ✅ |
| 举报内容或用户 / 拉黑用户 | ❌ | ✅ | ✅ |
| 编辑共修公告 | ❌ | ❌ | ✅ |
| 维护功课主清单 / 发布通知 / 活动 / 上传音视频 | ❌ | ❌ | ✅ |
| 处理举报 / 封禁用户 | ❌ | ❌ | ✅ |
| 学修问答(私密提问,§16) | ❌ | ✅(仅见自己的) | 全部可见、可回复、可删除 |
| 删除自己的账号 | — | ✅ | ✅ |

- **无"群"概念**:全 App 只有一个共修体(§3),注册即为其成员,报数与统计都以该共修体为单位。
- **未登录界面**(v0.5.8 定案):与登录后**几乎一致**(同一套首页分组宫格);点击账号类功能(報數 / 快捷報數 / 共修報數 / 個人統計 / 我的發願 / 通知 / 學修問答)时**强制跳转登录页**,登录后方可报数;直播 / 經本 / 日曆 / 工具 / 設定匿名可用。

---

## 3. 共修体模型(单一,v0.6.0 定案)

> **改动缘由**:原多群模型(输群 ID → 附申请说明 → 群主审核)是为"面向大众任何人都能建群"设计的;实际场景只有**善护念一个每日报数群**,而群 ID 不便在微信/Line 传播、年长用户记不住也不会操作。故取消"群"概念。详细设计与实施跟踪见 [`design/single-community.md`](design/single-community.md)。

### 3.1 入会流程(零门槛)
1. 用户注册登录(**用户名 + 密码**,v0.5.9:面向年长/无邮箱用户,注册零门槛;邮箱为选填,仅用于找回密码)。
2. **注册即自动成为共修体成员,立刻可以报数**——无群 ID、无申请说明、无审核等待。

### 3.2 共修体
- 全 App 只有一个共修体,App 内以「**共修報數 / 共修报数**」呈现(v0.6.0 用户定案;`groups.name` 同名,管理员可在后台改)。与日課组的「**報數**」区分:「報數」是**提交我的功课**,「共修報數」是**看数字与记录**。
- **共修公告**:一条常驻置顶公告(如本期共修安排),由 App 管理员在管理后台编辑,展示于「共修報數」页顶部;更新时生成一条 App 内通知。
- **共修报数记录**:全体注册用户可见(与原"群内可见"语义一致,规模放大);逐条可举报,可拉黑发布者。
- "共修整体统计" = 全体成员的汇总(含已删号用户匿名化保留的历史贡献,§10.1)。
- **管理**由 App 管理员承担:公告、功课主清单治理、封禁/解封、删除任意报数记录、处理举报(手机端 + 管理后台 §15)。
- 成员被封禁(`banned_at`)后不可报数/提问;删账号见 §10.1。

### 3.3 新人如何加入
**没有"加入"这一步** —— 装上 App、注册,就已经是共修体成员,可以立刻报数。不再有任何需要口头转述或转发的东西(群 ID、邀请码、邀请链接一概没有)。

App 的分发口径见 §14.4(海外走 App Store / Google Play,大陆 Android 走官网 APK 下载页);**App 内不做邀请 / 二维码功能**(v0.6.0 定案:让人知道并装上 App 发生在 App 之外,在 App 内放"拉人"按钮并不改变分发渠道)。将来官网下载页上线后,可在设置页加一个「分享下載連結」,属 P5.4 顺带项。

### 3.4 数据层取舍(实现说明)
数据库保留 `groups` / `group_members` / `practice_logs.group_id` 结构,共修体为**唯一有效行**(`groups.is_default`),`group_id` 恒指向它。这样保留了"将来若真要按地区/期别分组"的可逆性,也避免一次不可逆的破坏性迁移。**产品层面不存在群,亦不提供建群/入群入口**(服务端已关闭 `groups` 的 insert 与 `join_group` RPC)。

---

## 4. Epic 1 · 功课报数(核心)

### 4.1 功课清单
- **功课项必须具体**(v0.5.2 修正):經/咒/懺是**分类**而非功课项,不能笼统报"诵经 2 部"——报数对象是具体条目,如金剛經 1 部、地藏經 3 部、大悲咒 2 遍、地藏懺 1 遍。
- 每个功课项含:名称(简/繁双语)、**分类**(`經 / 咒 / 懺 / 念佛 / 靜坐 / 其他`)、**计数单位**;清单与报数选择器按分类分组展示,统计可按分类拆分。
- **主清单**由 App 管理员维护(全局);**任一注册用户可添加自定义功课项**(需选分类与单位)。**v0.6.0 定案:自定义项仅创建者可见**——只出现在**创建者自己**的功课选择器与清单里,不污染公共清单;但其**名称对所有人可读**(否则该项的报数在共修记录/总量里会成为无名条目)。用它报的数**照常计入共修总量**。管理员可在后台停用 / 改名 / 合并重复项 / 提升为全局主清单项。
- **计数单位**:`部`(经)、`遍`(咒 / 忏)、`次`、`分钟`(打坐等);单位挂在具体条目上,同类条目可用不同单位。

### 4.2 报数
- 提交当日 / 当次功课数量。
- 一条报数记录含:报数人、**被报对象**、功课项、数量、单位、备注、报数时间。
- **代报**:用户可帮任何人报,**被报对象不限于已注册同修**。对象选择器三个来源合一:
  - ① **同修**(v0.6.0:改为**搜索**——输入姓名关键字经 `search_members` RPC 查询,结果 ≤20;原"群成员全量下拉"在全站规模下不可用,且等于把全站用户目录发给每个客户端);计入其个人统计;
  - ② **代报名单**(之前用过的自由名字,点选即用);
  - ③ **直接输入任意新名字**(仅计入共修总量,不绑账号)。
  - **自由名字记忆**(v0.5.1;v0.6.0 起范围为全站):输入过的自由名字自动存入**代报名单**,下次输入时自动补全 / 点选;名单**全体同修共享**(避免同一人被写成多种写法,总量好归口);名单条目可由添加者或管理员删除(处理输错的名字,已产生的报数记录不受影响)。
  - **代报通知**(v0.4 补):被代报的**注册同修**会收到一条 App 内通知("某某为您报了 X");被代报人可**删除**该条记录(数据主权归本人)。自由名字对象无账号,不涉及通知。
- **补报("报之前的")**:随时补记忘记报的功课,**统一计入报数当天的统计**(不做历史日期归属);如需说明实际日期可写在备注。
- **报数交互定案**(v0.5.3,用户批准):
  - **批量多选**:一次报多种功课,每项独立数量(默认上次报的值,带 ± 步进按钮);
  - **首页「報數」直达**(v0.6.0):**直接进表单**(去群化后不再有选群分支);
  - **首页分组宫格**(v0.5.5;v0.6.0 换一格):功能按语义分组呈现——**日課**(報數、快捷報數)/ **共修**(直播、經本、**共修報數**、活動日曆、學修問答)/ **修行**(個人統計、我的發願、打坐計時、念珠計數)/ **通用**(通知、設定);日課组用大色块强调,布局大气简约、充分利用空间;通知与設定同时保留右上角小图标;
  - **快捷报数可多选**(v0.5.5):点开底部弹层,常用组合多选 + 核对各项数量(默认上次值,± 步进)后**一次提交**;
  - **「重複上次」**:一键带入上次(最近一天)自报的功课组合与数量;
  - **常用功课置顶**:选择区最前为「常用」组(按我最近报过排序),其余按分类分组;
  - **对象选择折叠**:默认报自己,点「替他人報數」才展开三来源选择器;
  - **提交栏固定底部**(显示已选项数);**提交成功展示随喜反馈 + 本次摘要**。
  - **报数记录按批次分组显示**(v0.5.10):**同一次提交**的多条功课**一起显示为一张卡片**(卡头为对象·日期·代报人·备注,卡内逐条列功课与数量),不再一条一条平铺;批次判定 = 同一报数人 + 同一对象 + 同一提交时刻(`created_at`,单条多行 insert 的事务时间);修改 / 删除 / 举报仍按单条(log id)生效。应用于共修报数记录页与个人统计的历史明细。
- **修改 / 删除**(v0.4 补):
  - 报数人可修改 / 删除**自己提交的**记录(含代报出去的);
  - 被代报人可删除**以自己为对象**的记录;
  - App 管理员可删除任意记录(处理恶意灌水;v0.6.0 起原"群主"权限归管理员);
  - 删除为**软删除**(`deleted_at`),统计即时扣减,后台可审计。
- **快捷报数**:首页常用功课一键报数;记住每个功课上次填的数量作默认值(老年用户友好)。

### 4.3 统计与 Dashboard
- **只显示个人数据与共修总量,不做任何排名 / 对比。**
- **统计时区口径**(v0.4 明确):每条报数落库时按**报数人当时的本地时区**计算出 `local_date`(自然日),所有"今日 / 按日趋势"统计一律以 `local_date` 聚合 —— 个人与共修总量共用同一口径,"今日" = 各成员"自己的今天"之和。
- **「我的」与「全體」都要有**(v0.6.0 用户要求):「共修報數」页的**今日 / 近 14 天趋势 / 累计**三处一律**并排双栏**(我的 / 全體);个人**明细**(按日期回看每条记录、连续用功天数、发愿进度)仍在「個人統計」页。双栏是**自己 vs 整体**的对照,不是成员间对比,不违反不排名基调。
- Dashboard 指标:
  - 个人:今日 / 累计,按功课类型拆分。
  - **个人连续用功天数**:连续有报数的自然日(按 `local_date`),**仅自己可见**、不进共修统计;中断只温和归零,不做"断签"提醒(守住不攀比、不施压的基调)。
  - 共修整体:今日已报人数 / 今日总量 / 累计。
  - **近 N 天趋势图**。
  - **发愿目标进度**(见 4.4)。
- **历史查看**:按日期 / 功课类型回顾过往报数。
- **管理员视角**:管理员同样只看总量与"今日已报人数"等聚合指标,**不提供成员明细排行**(守住不攀比基调);处理举报等管理需要时可查单个成员的记录列表。
- **数据导出**:本轮不做(留作备选:管理员导出时间段汇总 CSV,用于法会回向名单)。

### 4.4 发愿 / 立目标
- 用户可发愿:某功课在某周期内念满目标数量(如"90 天内念咒 10 万遍")。
- **进度归属**(v0.4 明确;v0.6.0 简化):统计该用户该功课项在周期内的报数总和(含别人代报给自己的)。~~也可在发愿时限定单个群~~ —— 去群化后范围选择器取消,`vows.group_id` 恒为 null。
- 发愿状态:`active / completed / expired / abandoned`;到期未完成不提示"失败",仅温和显示进度(文化基调)。
- Dashboard 展示**目标进度条**(已完成 / 目标、剩余天数);**达成时显示随喜祝贺页 + 回向文**(回向文文案由内容方提供,存后台可配置)。
- 发愿仅本人可见,不对外公示(不攀比)。

---

## 5. Epic 2 · 通知 / 活动 / 日历

- **循环模板 + 单次修改**:周六共修、周三打坐等按循环规则自动生成,单次可改时间/内容/取消。
- **通知类型**:周六共修预告(提前一天)、周六当天 Webex/YouTube 连接、周三打坐提醒、讲法/共修/法会预告。
- **活动提醒**(v0.5.21 定案,设计 [`design/notification-overhaul.md`](design/notification-overhaul.md) §2/§6):提醒点存 `event_reminders`(每活动可配多个提前量),**新建与存量活动默认三档 = 提前一天(1440 分) / 提前 30 分钟(30) / 活动开始时(0)**;可选档位 0/10/15/30/60/180/1440/2880 分,管理员可按活动增删。三档各司其职,缺一不可:
  - **提前一天** —— 让用户安排时间,且是**大陆 Android 用户唯一能看到的一档**(§5.1:该群体收不到实时推送,只能靠"打开 App 即见";只有覆盖 24 小时的预告才有机会被看到),也是本节邮件兜底的唯一有意义载体;
  - **提前 30 分钟** —— 临门一脚的准备提醒,补上预告到开始之间 24 小时的空档(不用 10 分钟:与"开始时"挨得过近,且用户群年龄跨度大,从看到通知到完成准备需要更多时间);
  - **活动开始时** —— 点击直进 Webex/YouTube。
  - 排程由 pg_cron 每日展开未来 14 天生成(§12.4);活动改期/取消/删除时未发提醒自动重排,已发的不追回。
- **跨时区**(v0.5.15 增强):活动时间以 `timestamptz` 存储,**按用户设备时区显示**;**管理员建/编辑活动时显式选择活动时区**(IANA 名,存 `events.timezone`,墙钟时间按该时区转 UTC),**默认时区管理员可配**(`app_settings.default_event_timezone`,初始 Asia/Shanghai);每周循环在**活动时区**做日历算术展开(跨夏令时保持当地墙钟不变);活动详情页当地时间与设备时间不同时加注「活動當地時間」。
- **日历视图**:所有活动可在日历查看;**未来事件列表**(v0.5.7:日历下方按时间列出即将到来的活动);同一天多种活动共存并全部显示。
- **事件类型**(v0.5.7 定案):动态表 `event_types`,默认 **靜坐 / 共修 / 講法 / 禪七 / 其它**;**管理员可增删改类型**(名称简繁 + 图标 + 启用状态,图标从预置图标集选择);不同类型在日历中显示不同图标;被活动引用的类型不可删、只能停用。
- **管理员活动管理**(v0.5.7):新增 / 编辑 / 删除整个活动(含循环全部场次)/ 取消单次;**任何活动变更自动生成全员通知**(新增/更新/删除/单次取消,App 内通知中心即时可见,推送接通后同链路升级)。**变更通知降噪**(v0.5.21):新增/删除立即发,编辑类走 5 分钟防抖窗口(窗口内反复编辑只聚合成一条);撤销单次取消/改期时补发「單次恢復」通知;同一活动的连续变更在手机通知栏折叠为一条;编辑器提供「通知所有人」开关(默认开),关闭后本次改动不发全员通知。**新建活动默认不重复**(v0.5.10:「每週重複」开关默认关闭,单次活动为主;需要每周循环时管理员手动开启)。
- **活动时间表与相关资料**(v0.5.12 定案,详细设计见 [`design/event-agenda.md`](design/event-agenda.md)):管理员可给活动补一份**时间表**(几点到几点做什么,**支持跨多天**如禪七)与**相关资料**;用户可看、可**整张时间表分享/复制**转发 Line/微信。
  - **时间表**:结构化行(第几天 + 起讫时间 + 活动),每行可选填一个**自由网址**(如读经行链接到经文网页);归属活动本身,循环活动各场次共用;时间为现场墙钟时间原样显示、不做时区换算。
  - **相关资料**:管理员**在 App 内上传 PDF**(如经本)供用户下载 —— 存 **Supabase Storage**(`event-files` bucket,公开可读、仅管理员可写);走自有域名 `api.pure-thoughts.com`,**大陆可达**(相对 YouTube 的关键优势)。另沿用活动级 YouTube/Webex 链接。
  - **分享**:整张时间表渲染为纯文本(含各行链接、PDF 公开 URL、YouTube),经系统分享面板发 Line/微信,或复制到剪贴板;转发后大陆用户点 Storage 链接可下 PDF。
  - **日历列表标记**:活动若含**时间表 / PDF / 链接**,在日历列表项(当日列表与未来活动列表)上分别挂对应小图标(时钟 / PDF / 链接),用户不点开即知里面有内容。
  - 均**管理员维护、用户只读**(匿名可看);改时间表/资料不额外发全员通知。
- **分类订阅**(v0.5.21 落地):用户可按通知类型开关,存 `notification_prefs.muted_types`(数组,随账号跨设备同步)。语义 = **不推送 + 通知中心不显示 + 不计红点**(与既有佛历开关行为一致,避免"关了还在列表里"的困惑)。首批可开关类型:活動提醒 / 活動變動 / 佛教節日 / 十齋日 / 學修問答回覆。

### 5.2 佛教日曆(纪念日 / 节日 / 十斋日,v0.5.15 新增,详细设计见 [`design/buddhist-calendar.md`](design/buddhist-calendar.md))

- **日历叠加佛历层**:月视图每格显示**农历副标签**(初一显示月名);佛教节日显示金色短名、十斋日有标记;选中日在活动列表上方显示**当日佛历卡**(农历全称 + 节日全名 + 十斋日)。
- **节日范围**:精选 24 条主要佛菩萨纪念日(佛诞/成道/涅槃/观音三日/弥陀/地藏/腊八/佛欢喜日等,重大节日标 ★)+ **十斋日**(农历每月初一、初八、十四、十五、十八、廿三、廿四、廿八、廿九、三十;小月无三十);清单为生成器配置,内容方可审改(改后重新生成 + 发版)。
- **数据来源(离线)**:`tools/almanac/` 生成器(6tail 官方 lunar Dart 库)一次性预生成 **2026–2075 共 50 年**逐日农历 + 节日 + 十斋日数据:客户端打包为资产文件(每年一个 JSON,**完全离线可读、匿名可见**),服务端同源生成 `almanac_days` 表(仅特殊日)供通知排程——**单一生成器保证两端口径一致**,客户端零农历运行时依赖。
- **时区语义**:农历日按中国时间(UTC+8)定义、映射为公历日期后**全球同一天显示**(全天性质,不随时区平移)。
- **首页横幅**:当天为节日/十斋日时,首页顶部(登录/未登录一致)显示莲花横幅「今日 · 農曆四月初八 · 釋迦牟尼佛聖誕(浴佛節)」,点击进日历;非特殊日不占位。
- **通知**:pg_cron 每日(UTC+8 零点后)生成全员通知(`type=almanac`,payload 携带简繁名/kind,幂等):节日与十斋日**当天**各一条,★ 重大节日**提前一天**加预告;通知中心即时可见,推送(P2.1)接通后同链路自动升级;设置页「佛教節日提醒 / 十齋日提醒」两开关默认开、可分别关闭。**开关上云**(v0.5.21):原实现仅在客户端过滤列表、服务端照推(用户关了仍收推送),现改为存 `notification_prefs.muted_types`(`almanac:festival` / `almanac:zhai`),服务端投递时即过滤;老用户首次登录时把本地两个开关一次性迁入。⚠️ 该 cron 在 UTC+8 零点后触发,对中国大陆用户即凌晨 00:05,**必须配合免打扰顺延**(见上条),否则推送上线即在半夜叫醒用户。
- **免打扰时段**:默认 22:00–07:00(用户本地时区)不发系统推送,顺延到时段结束后发;活动开始前的实时通知(如"共修连接")不受限;用户可在设置中调整或关闭。
  - **落地细则**(v0.5.21,原为纯需求描述、无实现):配置存 `notification_prefs`(`quiet_enabled` 默认 true / `quiet_start` 22:00 / `quiet_end` 07:00,支持跨午夜与同日两种窗口),按 **`profiles.timezone`** 判定(客户端登录时随其它偏好一并上报,此前从不同步);**"不受限"的判据 = `event_reminder` 且提前量 ≤60 分钟**(即 30 分钟与开始时两档放行,提前一天的预告遵守免打扰)。顺延实现:命中免打扰的用户不即时推送,由服务端克隆一条仅推送(不进通知中心)的用户级通知,排程到其本地时段结束时刻。

### 5.3 管理员发布通知 + 首页管理区(v0.5.16 新增,详细设计见 [`design/admin-notifications.md`](design/admin-notifications.md))

- **首页「管理」分组**:仅 App 管理员可见(普通用户不渲染);首批:發布通知、檢舉處理(自设置页迁入)。
- **发布通知**:标题 + 多行文字内容;可**定时发送**(按管理员设备本地时间);提交前预览确认;经 RPC `admin_publish_notification` 写入(notifications 表维持客户端不可直写),走既有通知中心 + 推送链路。
- **已排程管理**:未发送的定时通知可查看/取消;**撤回已发**:删行使所有人通知中心消失(已弹出的手机推送无法收回,界面注明);撤回/取消仅限 general 类型。
- 定时通知到点前不出现在用户通知中心(v0.5.16 一并修复查询口径)。
- **排程队列状态**(v0.5.21):列表区分「排程中 / 已发送 / 投递失败」三态(此前只能靠两个时间戳推导、无失败态);投递失败的条目显示重试次数与最近错误摘要。定时时刻**不再受"24 小时内创建"限制**——原实现中定到 24 小时以后的通知会永久发不出去(见 §5.4)。

### 5.1 推送策略(重要:约 1/3 用户在中国大陆)

Supabase 不发推送,由 Edge Function / DB 触发外部通道。**不接国内厂商推送**,分三层:

| 通道 | 覆盖 | 说明 |
|---|---|---|
| **APNs(原生,不经 FCM)** | 全部 iOS(含大陆) | APNs 在大陆可用;不走 FCM 以免注册拿不到 token |
| **FCM** | 海外 Android | 大陆 Android 收不到(依赖 Google 服务) |
| **App 内通知中心(拉取式)** | **全部,含大陆 Android** | 通知存 Supabase,开 App 即拉取 + 红点角标 |

- ⚠️ **关键后果**:**大陆 Android 用户收不到实时系统推送**,只能靠"打开 App 即见"→ 因此 **App 内通知中心是刚需,不是增强项**。
- **邮件兜底**:时效性强的通知(如周六共修预告)对大陆用户额外发邮件(经 Resend / SES 等,由 Edge Function 触发)。
  - **如何识别大陆用户**(v0.4 补):注册 / 设置中让用户自选"所在地区"(大陆 / 其他);另对 **FCM token 注册失败的 Android 设备自动标记**为需邮件兜底。
  - **判据修正**(v0.5.21):原设计的 `push_tokens.fcm_failed` 不可行——大陆机 `getToken()` 直接失败,`push_tokens` 里**根本没有该设备的行**可标记,该字段生产恒为 false。判据改为 **`notification_prefs.push_unavailable`**(按用户一行,客户端 FCM 注册失败时主动置位,成功时置回);`fcm_failed` 列保留但标注废弃。
- 未来若确需大陆 Android 实时推送,再评估接厂商通道(当前明确不接)。

### 5.4 通知中心与投递可靠性(v0.5.21 新增,详细设计见 [`design/notification-overhaul.md`](design/notification-overhaul.md))

**投递可靠性**(修复三个会在生产暴露的缺陷):

- **定时通知不再永久卡死**:原取数条件同时要求"创建于 24 小时内"与"已到排程时刻",定到 24 小时以后的通知到点时已滑出窗口 → 永不投递且永久计入积压告警。窗口基准改为**排程时刻**(到点后 6 小时内有效,防服务长时间宕机恢复后的过期雪崩)。
- **失败可重试、可见**:原实现在发送**之前**就标记已发送,失败不回滚、不重试、不留痕迹 → 一次网络抖动即静默丢失。改为租约式抢占(`claimed_at` 2 分钟租约 + `attempts` 计数),失败释放租约由每分钟 cron 自动重投,连续 5 次失败记为终局失败并在后台可见。重试粒度为**整条通知**而非单个设备,极端情况下可能对已成功用户重复推送一次,由通知栏折叠兜底(取舍详见设计文档 §4.3)。
- **投递渠道字段生效**:`notifications.channels`(inapp/push/email)此前两端均未生效,现由服务端按此决定是否推送、客户端按此决定是否进通知中心;这也是 §5.1 邮件兜底(P2.2)的前置。

**通知中心**:

- **已读语义**:由"进入页面即全部标记已读"改为**点击单条标记已读 + 顶部「全部已讀」按钮**,使未读红点真实反映未读量(原实现会把用户没细看的通知一并清掉)。
- **分页与时效**:游标分页(每页 30,触底加载,原为 50 条硬上限无分页);订阅 Realtime 使 App 前台时红点即时变化(原仅冷启动/下拉刷新时更新)。
- **时间显示**:按设备本地时区格式化(原直接截取 UTC 字符串显示)。
- **保留期**:通知保留 **180 天**,由每日 cron 清理(级联清除已读记录);此前无任何清理机制。

**深链**:推送与通知中心条目点击后直达对应页面(活动提醒/变动 → 该场活动详情、直播 → 直播页、问答 → 该会话)。此前点系统推送只是把 App 拉起到当前页,通知中心的活动通知也只能落到日历列表。

---

## 6. Epic 3 · 在线视频与往期录制

- **固定频道**(v0.5.6 定案):YouTube `youtube.com/@善護念`;Webex `purethoughts.my.webex.com/join/Shanhunian`;日历活动的直播链接默认预填这两个频道。
- **开播自动检测**(v0.5.6):`live-probe` Edge Function 探测 YouTube 频道 `/live` 端点(无需 API key)→ 开播时写 `live_streams` 表并生成全员通知(App 内通知中心即时可见;P2.1 推送接通后同链路升级系统推送);打开直播页时客户端触发探测,生产部署后 pg_cron 每 5 分钟服务端探测。**Webex 无公开状态 API**,不做自动检测:固定房间一键加入 + 日历活动预告为准。
- **直播入口**:YouTube / Webex。
- **首页直播角标**(v0.5.13):有进行中直播时,首页「直播」宫格图标右上角显示**红底白字「直播中」角标**提醒用户。数据取自 `live_streams` 表(轻量、无副作用,不触发 YouTube 探测、不建通知),反映 `live-probe`(生产由 pg_cron 每 5 分钟维护)写入的当前状态。
- **播放方式**:**YouTube 内嵌播放**;**Webex 应用内 WebView 加入**(v0.5.7 定案:媒体权限桥接支持网页版通话;App 显示名尽力预填访客名,失效则手动填、WebView 会记住;**所有 Webex 入口永远保留「用 Webex App 開啟」选项**——网页版兼容性风险的兜底)。
- **往期回看**:YouTube 存储的录制视频。
- **讲法问答检索**(v0.5.11 定案,详细设计见 [`design/qa-search.md`](design/qa-search.md)):对接内容方已有的问答检索 API
  (`www.pure-thoughts.com/api`,FastAPI;一条记录 = 一个**问答片段**,含 AI 摘要正文 + 带时间戳的 YouTube 链接)。
  - **客户端直连,不走 `qa-proxy` 代理**:该接口完全公开、无认证、CORS 全开,代理的两个立项理由(隐藏上游地址、统一鉴权)均不成立;直连少一跳、少一个故障点,且问答与 Supabase 无数据关系,不该被它拖挂。基址常量在 `core/channels.dart`,日后若需保护再引代理只改一处。
  - **简繁归一化在后端做**:库内简繁混杂(实测 `禅修`→6 条、`禪修`→1 条,互不包含),而 App 默认繁体 —— 不治理则搜索对默认语言用户基本不可用。方案:入库时 OpenCC 生成 hans/hant 双字形 + 简体归一检索列,`/search` 与 `/tags` 加 `script=hans|hant` 参数(响应字段名不变)。**此为客户端实现的硬前置**(PLAN E14)。
  - **详情页以播放器为主、摘要正文在下**;受 YouTube 嵌入限制(错误 153,真机已验),播放形态为「封面 + 大播放按钮 → 全屏 `/watch` 页并跳到该问题的时间戳」,非内嵌 iframe。
  - **摘要正文永远完整渲染**:大陆用户播不了 YouTube,但接口与摘要可达 —— 摘要是他们能拿到的全部价值,不得因播放失败而缺失(§14.2)。
  - 标签用**可搜索的选择器**(归一后约 600 个,全量 chips 不可用);多标签为「或」关系。匿名可用。
- ⚠️ 大陆可达性风险见 §14。

---

## 7. Epic 4 · 在线经本 / 资料库

- **经本网站**(v0.5.6 定案):`qldazangjing.com`(乾隆大藏經);`scriptures` 表由管理员维护条目(可加具体经文页链接)。
- App 内通过 **WebView** 查阅已有网站上的共修经本 / 其它经文;**App 字号设置透传到网页**(阅读缩放)。
- 建议:支持字号调节透传、可选离线缓存(后续增强)。

---

## 8. Epic 5 · 经典念诵 / 导引(音视频)

- 内容:经、咒、共修、静坐 的音 / 视频。
- **存储**:视频走 YouTube;**音频从指定 HTTPS endpoint 下载**(内容方自行托管 / CDN);App 负责下载、本地缓存、删除。媒体目录(标题、分类、URL)存 Supabase `media_items` 表,便于后台上架 / 下架。
- **下载管理**:在线下载保存本地,可删除以省空间;显示已用空间。
- **后台播放**:支持**息屏 / 锁屏后台播放 + 锁屏控制**(念诵、打坐必需)。
  - 注:自有音频用 `just_audio` + `audio_service` 可后台;YouTube 音频受其 ToS 限制不做后台。
- **播放模式**:**单曲循环 / 列表 / 定时停止(睡眠定时器)**,用户可选。

---

## 9. Epic 6 · 工具

### 9.1 计时器(打坐倒计时)
- 倒计时设定:**预设时长**(5 / 10 / 15 / 20 / 30 / 45 / 60 分钟)+ **自訂任意时长**(v0.6.1 用户需求)——时 / 分双滚轮选择,范围 **1 分钟 ~ 12 小时**,不用键盘输入(避免长者误输);选定后**本地记住**,下次进入沿用(预设与自订同等对待)。
- 计时中显示:< 1 小时为 `MM:SS`,≥ 1 小时为 `H:MM:SS`。
- 铃声:**预备铃 / 中途铃(每 X 分钟一声)/ 结束铃**,均可开关、可选铃声。("定点发声正念提醒"即中途铃。)
- 计时结束后可**一键把本次分钟数转成打坐报数**(v0.6.0 起无需选群,直接提交)。

### 9.2 计数器(念珠计数)
- **交互**:全屏大按钮点按计数(屏幕任意处 + 明显的 +1 按钮),iOS / Android 一致;可设屏幕常亮防误触。
- **不使用音量键**(避免 iOS 拒审与实现不稳定)。
- 计数可清零、可保存;**一键转成报数**(选功课项后提交,便于闭环)。
- **整串提示**:每满一串(默认 108,可自定义)震动 / 轻响一次,可关闭。

### 9.3 正念提醒
- 定时 / 定点发声提醒正念(与计时器中途铃共用铃声机制);本地通知实现,无需服务端。
- **OS 级版本(设计定稿 2026-07-14,待实现;详见 `docs/design/mindfulness-reminders.md`)**:用户设周几(默认全选可改)× 时间窗(同日,不跨午夜)× 间隔(≥10 分钟),App 关闭/息屏也能定时响磬——用**每周重复本地通知槽**实现(安卓无上限 / iOS ≤64 永久生效,超限提示降级);通知文案用户自定;**尊重系统静音/勿扰**(不穿透);提供 iOS/Android **设置帮助页**(含大陆厂商自启动/电池引导);**双端体验一致**。技术边界诚实告知:iOS 无 Critical Alerts、大陆厂商可能杀后台。

---

## 10. 合规与账号(v0.4 新增,MVP 必做)

> 以下为 App Store / Google Play 上架的**硬性要求**,不是增强项。

### 10.1 账号删除
- Apple 政策:提供注册就必须提供**应用内删除账号**入口。
- 流程:设置 → 删除账号 → 二次确认 → Edge Function `delete-account` 执行:
  - 删除 `auth.users` 与 `profiles`(级联清除个人资料、push token、发愿、通知已读等);
  - **报数记录匿名化**而非物理删除:`reporter_id / subject_user_id` 置空、显示名替换为"已注销用户"——**共修总量不变**(共修功德数据保留,符合文化基调;**已确认**)。

### 10.2 UGC 举报与拉黑
- 本 App 含 UGC(自定义功课名、报数备注、自由名字、显示名、学修问答),Apple Guideline 1.2 要求具备:
  - **举报**:对用户 / 单条记录可举报,进入管理员处理队列(`reports` 表);**v0.6.0 起"群"不再是举报目标**(`report_target` 枚举保留 `group` 值不再使用),入口统一在**共修报数记录的条目菜单**;
  - **拉黑**:用户可拉黑某用户(不再看到其代报名字、其报数记录、其在搜索中出现);
  - **管理员处置**:警告 / 删除内容 / 封禁账号(`profiles.banned_at`);
  - 社区规范文案 + EULA 同意(首次启动勾选)。
- v0.3 遗留的"开放建群防滥用机制"随 v0.6.0 去群化**自然消解**(不再有建群);防滥用现为举报 + 管理员处置 + 封禁。

### 10.3 隐私政策与权限声明
- 隐私政策 / 服务条款网页链接(设置页 + 商店链接)。
- iOS 推送权限、通知权限按需申请并解释用途;App Privacy 标签如实填报(收集:显示名、使用数据、邮箱(选填,仅找回密码))。

---

## 11. 非功能需求 (NFR)

- **视觉基调**(v0.5.4 定案,参照官网):**宣纸暖白底 + 古铜金主色(#8A6D3B 系)+ 深棕文字**,沉静典雅;Material 3 双主题(浅色/深色/跟随系统,用户可切换),**深色不用纯黑**、金色降饱和;8pt 网格、宽松留白、正文行高 1.5;按钮三级层级(filled/tonal·outlined/text),触控目标 ≥48pt;加载用骨架屏、空态给引导文案与动作,不用裸转圈。
- **品牌标志**(v0.5.14 定案,母版 `docs/images/`、生成流程 `design/branding/`):正式徽记为**「托手莲花 + 祥云圆环」古铜金**(浅版宣纸暖白底 `logo (2).jpg`、深版墨黑底 `logo (1).jpg`);App 图标、启动屏、App 内运行时 logo 一律用此徽记(图标底为宣纸暖白 `#F6F1E4`,Android 自适应 + iOS + Android 12 启动圆形遮罩已适配)。替换了早期通用金蓮占位标。
- **多语言**:UI 维护 `zh_Hant` / `zh_Hans` 两套文案;用户可切换,默认繁体;偏好存云端 + 本地。
- **可访问性**:用户**自定义字号**(全局文字缩放),布局需自适应大字号;高对比、简洁层级;首次启动引导页(选语言 → 选字号 → 选地区)。
- **隐私 / 安全**:Postgres **RLS** 全表启用(见 §12.3);匿名用户仅能读公开内容表;客户端只用 `anon key`,管理操作走 Edge Function(`service_role` 仅存在于服务端)。
- **离线**:工具(计时 / 计数)、已下载音频离线可用;报数支持离线暂存后同步(增强项;同步冲突策略:报数为追加型记录,直接补传即可,无覆盖冲突)。
- **性能**:冷启动、列表滚动、音频播放流畅;包体积控制;统计查询用聚合视图,避免客户端拉全量记录。
- **可观测性**(v0.4 补):客户端接 Sentry(崩溃 / 错误上报);服务器 Uptime + 磁盘/CPU 告警;关键 Edge Function 失败重试与告警。
- **备份**(自托管,自行负责):每日 `pg_dump` 全量 + WAL 归档(如 wal-g)到异地对象存储,**定期做恢复演练**;详见 §12.5。

---

## 12. 技术架构(Supabase 自托管,v0.5)

**客户端**:Flutter · Riverpod(状态) · go_router · flutter_localizations/intl(简繁) · just_audio + audio_service(后台音频) · youtube_player_iframe(视频) · webview_flutter(经本) · table_calendar(日历) · firebase_messaging(仅海外 Android)+ 原生 APNs 注册(iOS) · supabase_flutter(Auth/DB/Realtime) · sentry_flutter。

### 12.1 Supabase 模块分工

| 模块 | 用途 |
|---|---|
| **Auth** | **用户名+密码**(v0.5.9 定案):用户名映射内部邮箱 `<用户名>@u.pure-thoughts.com`(含 `@` 的用户名视为真实邮箱账号);**注册免邮箱验证**(`ENABLE_EMAIL_AUTOCONFIRM=true`);选填恢复邮箱存 `profiles.recovery_email`;找回密码:真实邮箱账号走标准重置邮件,纯用户名账号由管理员协助重置;**Google/Apple 登录移出 MVP**(纯账密不触发 Apple 强制 Sign in with Apple 条款);匿名浏览不建会话,公开内容用 `anon` 角色直读 |
| **Postgres + RLS** | 全部业务数据;行级安全实现"个人明细仅本人可见、报数记录限注册用户可见" |
| **Edge Functions** | 推送、邮件、账号删除、问答 API 代理(见 12.4) |
| **pg_cron + pg_net** | 循环活动实例生成、通知排程、触发推送函数 |
| **Realtime** | (v0.2+)共修总量实时刷新 + 通知红点;MVP 先下拉刷新 |
| **Storage** | **活动 PDF / 经本**(v0.5.12:`event-files` bucket,公开读、管理员写,大陆可达);头像 / 群头像(可选);**念诵音频不放 Storage**(已定走内容方 HTTPS endpoint)。注:音频与 PDF 口径不同——音频量大且内容方已有 CDN 走外部 endpoint;PDF 需管理员在 App 内直传、量小,故用 Storage |
| **Database Webhooks** | `notifications` 表 insert → 触发 `push-dispatch` |

### 12.2 数据模型(Postgres)

| 表 | 关键字段(v0.4 补字段加粗) |
|----|---------|
| `profiles` | id(↔auth), display_name, locale, font_scale, **timezone**, **region(cn/other)**, is_app_admin, **banned_at**, **recovery_email(v0.5.9,选填找回邮箱)**, created_at |
| `groups` | id, name, ~~join_code~~, owner_id, description, **announcement**, **deleted_at**, **is_default(v0.6.0,唯一;= 共修体那一行)** —— v0.6.0 起只有 `is_default` 那一行有效,其余为历史软删行;`join_code` 独立表 `group_join_codes` 不再产生新值 |
| `group_members` | group_id, user_id, status(v0.6.0 起恒 `approved`;`pending/rejected/removed/left` 为历史值), role, apply_message(历史), approved_at |
| `practice_types` | id, group_id(null=全局主清单), name_hant, name_hans, **category(sutra/mantra/repentance/buddha_name/meditation/other)**, unit, is_custom, **created_by(v0.6.0,自定义项的创建者;仅其本人可选用)**, active, sort_order |
| `practice_logs` | id, group_id(**v0.6.0 起恒 = 共修体 id**), reporter_id, subject_user_id(nullable), subject_name(nullable), practice_type_id, quantity, unit, note, created_at, **local_date(按报数人时区的自然日,统计口径)**, **updated_at**, **deleted_at(软删)** |
| **`proxy_names`** | id, group_id(恒 = 共修体), name, created_by, use_count, last_used_at —— 代报自由名字的**共享名单**(v0.6.0 起全站共享);唯一约束 (group_id, name);报数时自动 upsert |
| `vows` | id, user_id, group_id(**v0.6.0 起恒 null**), practice_type_id, target_qty, start_date, end_date, **status(active/completed/expired/abandoned)** |
| `events` | id, title, type, start_at(timestamptz), duration, recurrence_rule(RRULE), **timezone(IANA 名,v0.5.15,循环展开与当地时间标注用)**, webex_url, youtube_url, content, created_by |
| **`almanac_days`**(v0.5.15) | solar_date(PK), lunar_month/day, is_leap_month, festival_ids[], names_hant[]/names_hans[], is_zhai_ten, has_major —— 佛历特殊日(由 `tools/almanac/` 生成器产出,仅供通知排程;客户端用同源资产文件) |
| **`app_settings`**(v0.5.15) | key(PK), value —— 全局键值配置;首个键 `default_event_timezone`(建活动默认时区) |
| `event_overrides` | event_id, occurrence_date, patch(改期/改内容/取消) |
| **`event_agenda_items`**(v0.5.12) | id, event_id(FK cascade), day_index(第几天,≥1), start_time/end_time(time,墙钟), activity, link_url/link_label(自由网址,可空), sort_order —— 活动时间表行;匿名可读、仅管理员写 |
| **`event_attachments`**(v0.5.12) | id, event_id(FK cascade), title, storage_path(`event-files` 桶内 key), size_bytes, content_type, sort_order —— 相关资料 PDF;删行前先删 Storage 对象(级联不清对象) |
| `media_items` | id, title_hant/hans, kind(audio/video), source(youtube/https), url, size, category, sort_order, active |
| `scriptures` | id, title, web_url, sort_order |
| `push_tokens` | user_id, token, platform(apns/fcm), ~~fcm_failed~~(v0.5.21 **废弃**:大陆机无 token 行可标,判据移至 `notification_prefs.push_unavailable`), updated_at |
| `notifications` | id, scope(user/group/all), target_id, title, body, type, event_id(nullable,**v0.5.21 起活动类通知真实写入**), **scheduled_at**, **sent_at**, **channels(push/email/inapp,v0.5.21 起两端生效)**, created_at, **claimed_at / attempts / failed_at / last_error**(v0.5.21,租约式投递与重试) |
| `notification_reads` | notification_id, user_id, read_at |
| **`notification_prefs`**(v0.5.21) | user_id(PK), quiet_enabled(默认 true), quiet_start(22:00) / quiet_end(07:00), muted_types(text[],分类订阅), push_unavailable(bool,FCM 不可用=需邮件兜底), updated_at —— 每用户一行的通知偏好,跨设备同步 |
| **`event_reminders`**(v0.5.21) | id, event_id(FK cascade), offset_minutes(提前分钟数), enabled, created_at —— 活动提醒点;每活动可多条,默认三档 1440/30/0,唯一键 (event_id, offset_minutes) |
| **`reports`** | id, reporter_id, target_type(user/group/log), target_id, reason, status(open/resolved), handled_by, created_at |
| **`user_blocks`** | user_id, blocked_user_id, created_at |
| **`qa_threads`**(v0.5.18) | id, user_id(FK cascade,删号连带清空), last_sender_role(user=待回覆/admin=已回覆), last_message_at, first/last_message_preview, user_last_read_at, created_at —— 学修问答线程(§16);冗余列由触发器维护 |
| **`qa_messages`**(v0.5.18) | id, thread_id(FK cascade), sender_id(删号 set null), sender_role(user/admin), body(≤2000 字), created_at —— 首问与全部往来消息,硬删随线程级联 |

> 统计:按 `local_date` 聚合;建 `daily_user_stats` / `daily_group_stats` **视图**(量大后改物化视图或 rollup 表)。`practice_logs` 建索引 `(group_id, local_date)`、`(subject_user_id, practice_type_id, local_date)`(发愿进度查询)。

### 12.3 RLS 策略要点(每表一句话)

| 表 | 策略 |
|---|---|
| `profiles` | 本人可读写自己;他人显示名仅经 definer RPC `search_members(q)` 窄口暴露(需关键词、≤20 条、排除封禁与已拉黑);管理员全读 |
| `groups` | 成员(= 全体注册用户)可读;**仅管理员可更新**;**v0.6.0 起不开放 insert**(建群关闭,`groups_insert` 策略删除 + `revoke insert`);`join_group()` RPC 停用 |
| `group_members` | 本人可读自己的记录;管理员可读写全部;insert 仅经注册触发器(自动入会) |
| `practice_logs` | 全体注册用户可读(`deleted_at is null`);insert 限未封禁的注册用户;update/delete 限 reporter 本人、subject 本人(仅删)、管理员 |
| `proxy_names` | 注册用户可读、可 insert(报数时自动记入);删除限添加者 / 管理员 |
| `vows` | 仅本人读写 |
| `practice_types` | **可读**:全局项所有人可读、自定义项注册用户亦可读(渲染他人记录的功课名所必需);**可选**:客户端与选择器一律过滤 `group_id is null or created_by = auth.uid()`(仅主清单 + 我自己加的);insert 限未封禁注册用户且 `created_by` 由触发器落 auth.uid();停用/改名/合并限管理员 |
| `events` / `event_agenda_items` / `event_attachments` / `media_items` / `scriptures` | **anon 可读**(匿名浏览),仅管理员写(v0.5.12:两活动子表 + `event-files` Storage 桶同口径,`storage.objects` 公开读、`is_app_admin()` 写) |
| `almanac_days` / `app_settings` | **anon 可读**;`almanac_days` 无人可写(仅 migration/service),`app_settings` 仅 `is_app_admin()` 可写(v0.5.15) |
| `notifications` | scope 命中者可读;仅服务端(service_role)写 |
| **`notification_prefs`**(v0.5.21) | 仅本人读写自己;管理员全读。⚠️ 建表时必须显式 `revoke all from public, anon, authenticated` 后再按需 grant(生产库新表默认授权会覆盖最小授权,见 §12.5 教训) |
| **`event_reminders`**(v0.5.21) | **anon 可读**(与 `events` 同口径,匿名可见活动设了哪些提醒),仅 `is_app_admin()` 可写 |
| `reports` | 举报人可 insert / 读自己的;管理员全权 |
| `user_blocks` | 仅本人读写 |
| `qa_threads` | 本人可读/建/删自己的;管理员全读全删;update 不开放(冗余列走触发器,已读走 RPC `mark_qa_thread_read`);insert 校验未封禁,待回覆 ≤3 由触发器拦截(v0.5.18) |
| `qa_messages` | 能读所属线程者可读;insert 限线程主人(role=user)或管理员(role=admin)且未封禁;不可单条改删,随线程级联硬删(v0.5.18) |

### 12.4 Edge Functions 与定时任务

| 函数 | 触发 | 职责 |
|---|---|---|
| `push-dispatch` | DB webhook(notifications insert)/ cron | 查目标用户 token,APNs(JWT 直连)+ FCM 发送;失效 token 清理。v0.5.21:租约式抢占(`claim_notifications`)+ 结果回写(`complete_notification`)+ 受众解析下沉为 `push_audience` RPC(内含 channels / 分类订阅 / 免打扰 / 封禁四道过滤)+ 免打扰顺延克隆 + 报文携带深链 route 与折叠 id |
| `email-fallback` | 由 push-dispatch 级联 | 对 region=cn 或 fcm_failed 的用户发邮件(Resend/SES) |
| `delete-account` | 客户端调用(需登录) | §10.1 删除 + 匿名化流程 |
| `admin-ops` | 管理后台调用(需登录,函数内验 `is_app_admin()`) | §15 需 `service_role` 的特权操作:重置任意用户密码、代删账号、设/撤管理员;CORS 仅放行 admin 子域 |

> ~~`qa-proxy`~~ **已取消**(v0.5.11):上游问答 API 完全公开、无认证、CORS 全开,「隐藏上游地址 / 统一鉴权」两个立项理由均不成立;客户端直连(§6)。

**pg_cron**(v0.5.21 落地并补全,此前只有下列第 2、3 项):

| job | 频率 | 职责 |
|---|---|---|
| `event-reminders-daily` | `0 15 * * *`(UTC) | `expand_event_reminders(14)`:按 `event_reminders` 的提前量,在**活动时区**展开未来 14 天 occurrence(RRULE 子集 + 应用 `event_overrides` 的取消/改期)→ 生成 `type=event_reminder` 通知(`scheduled_at` = 提醒时刻,幂等键 = event_id + occurrence_date + offset_minutes)。活动改期/取消/删除或提醒点增删时,由触发器即时清掉未发提醒并重排 |
| `almanac-daily` | `5 16 * * *`(= UTC+8 次日 00:05) | `generate_almanac_notifications()`(v0.5.15,幂等,重跑不重复) |
| `push-dispatch-sweep` | `* * * * *` | 兜底触发 `push-dispatch` 投递到点通知 |
| `notifications-retention` | `0 17 * * *` | `purge_old_notifications(180)`:清理 180 天前的通知(级联清已读记录) |

> **排程精度**:`push-dispatch-sweep` 粒度为 1 分钟,故所有提醒的 `scheduled_at` 统一提前 60 秒排程,避免"活动开始了"迟到一分钟。
> **展开口径一致性**:服务端 SQL 展开必须与客户端 Dart(`occurrence_utils.dart`)、管理后台 TS 三处口径一致——每周循环在**活动时区**做日历算术(跨 DST 保持当地墙钟),`occurrence_date` 取活动时区日期。

### 12.5 自托管部署要点(v0.5 新增)

Supabase 全套开源,自托管(官方 Docker Compose,或 Coolify 等面板)与云端 **SDK 完全兼容**:`supabase_flutter` 只需指向自有域名 + anon key,本章 RLS / Edge Functions / pg_cron 设计不变。**结论:可行,且机房选址可直接缓解大陆可达性风险**;代价是运维责任全部自担。

- **机房选址**:⚠️ **不要放中国大陆境内**(需 ICP 备案,且宗教内容境内托管有合规风险)。选大陆直连质量较好的境外机房:**香港 / 新加坡 / 东京**;上线前用大陆真实网络实测延迟与丢包后定。
- **暴露面**:只暴露 Kong 网关(443 + TLS,自有域名);Postgres、Studio 不上公网(Studio 走内网 / VPN / IP 白名单)。
- **Auth 自配项**:**SMTP**(重置密码邮件、§5.1 邮件兜底;v0.5.9 起注册不再依赖邮件,SMTP 非上线硬阻塞)——可用 Resend / SES。Google/Apple OAuth 已移出 MVP,将来如加回再配回调域名与密钥。
- **Edge Functions**:自托管 `edge-runtime` 容器运行 §12.4 四个函数;若维护不便,可退化为同机的一个小型 Node/Deno 服务 + 反向代理,职责不变。
- **pg_cron / pg_net**:官方 Postgres 镜像已含扩展,启用即可,排程链路不变。
- **备份与恢复**:每日全量 + WAL 归档到**异地**对象存储(B2/R2/S3);每季度恢复演练——自托管没有云端 PITR 兜底,"备份可恢复"是第一优先级。
- **升级与监控**:跟进 Supabase 镜像安全更新(先在测试环境验证再升级生产);Uptime 监控 + 资源告警。本 App 量级(数百至数千用户)单机足够,先保证备份,再考虑高可用。

---

## 13. 分期实现建议(Roadmap)

> 执行层面的详细任务拆解、验收标准(DoD)与进度跟踪见 [`PLAN.md`](PLAN.md);本章只保留版本级路线,两者以 PLAN 的任务状态为进度准绳。

- **v0.0(基础设施)** — 自托管 Supabase 部署(§12.5):机房实测选址、域名 + TLS、Auth(OAuth/SMTP)配置、备份链路跑通。**App 开发的前置条件**。
- **MVP (v0.1)** — 核心闭环 + 上架合规:Auth(用户名+密码,v0.5.9)、~~报数群(建群/入群审核/退群/移除/转让/重置群ID)~~(**已随 v0.6 去群化下线**)、公告、报数(代报/补报/改删/**快捷报数**)、个人+共修总量统计(**含连续用功天数**)、Dashboard、简繁切换、字号设置、**账号删除、举报/拉黑、隐私政策**。
- **v0.2** — 通知 / 活动 / 日历 + **推送基础设施(APNs/FCM/通知中心/邮件兜底/免打扰时段)**;发愿目标进度(**含达成随喜 + 回向文**);(可并行)工具:计时器 / 计数器(**含整串提示**)/ 正念提醒(技术独立、可离线,能更早出可玩版本)。
- **v0.3** — 视频(YouTube 内嵌 + Webex 跳转/内嵌)、经本 WebView。
- **v0.4** — 念诵导引音视频(下载管理、后台播放、播放模式)。
- **v0.5** — 打磨:离线报数暂存、Realtime 实时统计。
- **后续** — 讲法问答检索(待通用 API JSON);群数据导出 CSV(本轮未采纳,留作备选)。
- **v0.6(去群化)** — 取消"群"概念:单一共修体、注册即入、无群 ID、无审核;邀请靠下载链接 + 二维码;群主角色并入 App 管理员(§3,设计 [`design/single-community.md`](design/single-community.md),任务见 PLAN P9)。
- **管理后台(Web,v0.5.17)** — `admin.pure-thoughts.com` 静态站,与 App 并行推进,分两期(§15,任务见 PLAN P7)。
- **学修问答(v0.5.18)** — 私密提问 + 多轮问答 + App/后台双端回复(§16,任务见 PLAN P8);独立于 P2–P5 与 P7 余项,可并行。

---

## 14. 待确认事项 / 风险

1. ~~**讲法问答 API**:等你提供 JSON 接口示例 / 返回样例后再补设计。~~ ✅ **已关闭**(v0.5.11,2026-07-15):接口文档已提供并实测,设计定稿见 §6 与 [`design/qa-search.md`](design/qa-search.md)。余留一个**新依赖**:上游需做简繁双字形改造后搜索才对繁体用户可用(PLAN E14)。
2. ⚠️ **大陆用户的内容可达性**:约 1/3 用户在大陆,而 YouTube(视频/音频)与 Webex(直播)在大陆需翻墙。是否要为大陆用户提供备用内容源(国内可访问的镜像 / CDN、直播备用通道)?**暂缓,但需评估影响面**——这关系到 Epic 3/5 对 1/3 用户是否可用。
3. ⚠️ **后端大陆可达性**(v0.5 更新):已决定**自托管**,通过机房选址(香港/新加坡/东京)+ 自有域名缓解;**上线前必须在大陆真实网络实测**(登录、报数、拉通知)后定机房。服务器**不得放大陆境内**(ICP 备案 + 宗教内容合规,见 §12.5)。这影响的是核心功能(报数)对 1/3 用户的体验,优先级高于第 2 条。
4. ⚠️ **中国区 App Store 上架**:宗教类内容在中国区上架需 ICP 备案与宗教信息服务许可,大概率**无法上架中国区**。现实路径:大陆 iOS 用户使用外区 Apple ID 下载;大陆 Android 用户经官网 APK 直接分发(不依赖 Google Play)。需与内容方确认分发口径,并在官网准备下载页。
5. **自托管运维责任**(v0.5 新增):备份可恢复性、安全更新、监控告警均自担;上线前需完成一次完整的**备份恢复演练**(§12.5)。

**已定案**(四轮澄清;⚠️ 前两项已被 **v0.6.0 去群化推翻**):~~注册用户可自建群、建群者即群主~~ · ~~入群申请需附一条说明~~ · 补报统一计入当天 · 代报允许自由名字 · 音频从 HTTPS endpoint 下载 · 计数器仅用屏幕按钮 · 推送 = APNs(iOS)+ FCM(海外 Android)+ App 内通知中心兜底,**不接国内厂商推送** · 报数软删除可改可删 · 代报通知被代报人且其可删 · 统计按 `local_date` 单一口径 · 发愿默认跨群统计 · 账号删除 + 举报拉黑纳入 MVP · **删号匿名化保留群总量** · **自定义功课项群成员均可加** · **Supabase 自托管** · **采纳:快捷报数 / 群公告 / 发愿随喜回向 / 连续用功天数(仅自己可见)/ 计数器整串提示 / 免打扰时段;群数据导出暂不做** · **代报自由名字自动记忆成本群共享名单,供复用** · **账号体系 = 用户名+密码,注册免邮箱验证,邮箱选填仅作找回,Google/Apple 登录移出 MVP(v0.5.9,面向年长/无邮箱用户)** · **通知系统(v0.5.21)**:活动提醒默认三档 1440/30/0 且「提前一天」不可省(大陆 Android 唯一可见档)· 活动提醒发全站(不做活动报名,用分类订阅收敛)· 投递重试粒度为整条通知而非单设备(重复推送由通知栏折叠兜底)· 免打扰放行判据 = `event_reminder` 且提前量 ≤60 分 · 通知保留 180 天 · 已读改为点击单条 + 全部已读 · **去群化(v0.6.0)**:全 App 单一共修体,App 内叫「**共修報數**」,**注册即入、无群 ID、无申请、无审核**;群主角色取消并入 App 管理员;**不做 App 内邀请/二维码**(没有任何需要转述的东西,分发本就在 App 之外);**不为"谁能注册"另加门槛**(靠举报/拉黑/封禁事后治理);代报"同修"改搜索;共修报数记录全体可见(随喜,非排名);**自定义功课项仅创建者可选**(名称仍可读、报数照常计入共修总量);「共修報數」页今日/趋势/累计**均为「我的 / 全體」双栏**;**数据层保留 `group_id` 单例以保可逆**,产品层不存在群。

---

## 15. 管理后台(Web,v0.5.17 新增)

面向 App 管理员(`profiles.is_app_admin`)的桌面 Web 管理台。目标:覆盖手机端管理员的全部能力并桌面化增强,再补上手机端做不了的运维/内容/看板类功能;不影响 App 内已有管理入口。

### 15.1 已定案(2026-07-20)

- **技术栈**:Next.js(App Router,**静态导出 `output: 'export'`,无 Node 服务端**)+ React + shadcn/ui + Tailwind + supabase-js;仓库目录 **`admin/`**(与 `app/`、`supabase/` 并列,schema/RPC/类型生成强耦合,不分仓)。
- **部署**:子域 **`admin.pure-thoughts.com`**,纯静态文件托管在 API 同机 EC2,由 **Caddy** 加一个 site block 伺服(自动 HTTPS,无需 certbot;Caddy 是该机唯一前门,见 `infra/deploy-aws-ec2.md` §4;2026-07-20 依生产实况更正)。
- **安全模型**:管理员用自己的账号密码登录(复用 Supabase Auth,**不做 MFA**,用户定案);一切读写走 anon key + 登录 session,由现有 RLS / `is_app_admin()` / RPC 把关——与 App 同一套权限模型,后台不引入新特权面。需 `service_role` 的操作走新 Edge Function **`admin-ops`**(§12.4):函数内先验 `is_app_admin()`,处理浏览器 CORS 且仅放行 admin 子域(本地开发另放行 localhost)。
- 非管理员账号登录后台:提示无权限并登出,不渲染任何数据。

### 15.2 功能范围

**一期(对齐手机端管理员,桌面化增强)**

- 活动管理:events CRUD(RRULE 编辑 + 未来场次预览)、单次改期/取消(event_overrides)、议程与 PDF 附件(Storage `event-files`)、活动类型 event_types。
- 通知发布:立即/定时发布、排程队列一览、撤回(`admin_publish_notification` / `admin_cancel_notification`)。
- 举报处理台:reports 列表与处理流转、封禁/解封(`banned_at`)。
- 佛历 `almanac_days` **只读浏览**(数据仅来自生成器 migration,schema 即无人可写;调整清单走 E15 生成链路)+ `app_settings` 维护。

**二期(后台独有)**

- 用户管理:搜索、封禁/解封、**重置密码**(替代 deploy 文档 §10 的 psql 运维命令,走 `admin-ops`)、设/撤管理员、代删账号。
- **共修体管理**(v0.6.0 取代原"群总览"):共修体信息与名称、**公告编辑**、同修列表(搜索/封禁/代删)、共修总量;历史群列表降级为只读折叠区(供追溯)。
- 数据看板:每日报数量与活跃趋势、推送投递健康(v0.5.21 改读 `notification_prefs.push_unavailable`,原 `fcm_failed` 恒为 0 已废弃)、通知队列积压(`scheduled_at` 到点未 `sent_at` 且未终局失败)、**投递失败明细**(`failed_at` / `attempts` / `last_error`)、近 24h 投递结果汇总。
- 内容上架:media_items(P4 音频)、live_streams 频道、practice_types 主清单、scriptures。

### 15.3 非目标

- **不重造 Studio**:临时查数/改表仍走白名单内的 Studio;后台只做业务语义操作(带校验、二次确认、上下文)。
- 不做 MFA;不做多角色分级(仅 `is_app_admin` 一档)。
- 审计日志(`admin_audit_log`)为可选增强,单管理员阶段可缓;多管理员出现时再立项。

---

## 16. 学修问答(在线提问,v0.5.18 新增;v0.5.19 修订角色语义)

用户在 App 内向管理员提出学修问题的**私密一对一问答**,与 §6 的「讲法问答检索」(只读检索既有讲法内容)互补、完全独立。详细设计与完整测试计划见 [`design/study-qa.md`](design/study-qa.md),任务见 PLAN P8。

- **提问**:注册用户可提交问题(纯文字,单条 ≤2000 字);未登录点击入口跳登录(§2 口径)。入口:首页「共修」组宫格加一格「學修問答」。
- **可见性**:问题与全部往来**仅提问人本人与管理员可见**,其他用户不可见(RLS 实现)。
- **回答**:管理员(`is_app_admin`)可见全部问题并回复;**App 与管理后台(§15)双端均可回答**;管理员回复**署该管理员的显示名**(v0.5.20 定案:管理员之间有分工,提问人需要知道是谁回答的;发送者已删号或名字取不到时回退显示「管理員」)。消息名字经 `qa_message_display` definer 视图窄口暴露(普通用户读不了他人 profiles,同 `group_member_display` 纹路)。
- **消息角色(v0.5.19 定案,源于真机测试缺陷)**:每条消息的角色(提问人/管理员)按**发言时的身份语境**记录,**不得从"是否线程主人"推导**——管理后台回复恒为管理员身份;App 内从管理员视图(待回覆/全部)进入会话即以管理员身份发言,从「我的提問」或回复通知进入即以提问人身份发言。界面(三端)按**角色**分侧渲染并署名(管理员消息署该管理员显示名——v0.5.20,回退「管理員」;管理员语境下用户消息署「提問人」),管理员自问自答时谁问谁答也一目了然。管理员在 App 端列表为三个 tab:**待回覆 / 全部 / 我的提問**。
- **多轮**:一个问题是一个会话线程,回答后可**无限追问**,不设「关闭/已解决」状态;线程状态即「待回覆/已回覆」(以最后发言方区分)。
- **通知**:管理员回复 → 通知提问人(**含自答自己提问的情形**,v0.5.19 起不再抑制,保证多设备可达与可测);用户提问/追问 → 通知全体管理员(发送者本人除外)。走既有 `notifications` 链路(通知中心 + 推送 + 邮件兜底,数据库触发器生成);**通知与推送文案均不携带问题正文**(隐私)。
- **删除**:提问人与管理员都可删除问题——**硬删**,整个线程连同全部消息永久删除(不影响任何统计,私密内容不留档);账号删除时其全部提问级联清空。
- **防滥用**:被封禁用户不可提问/发消息;每人同时最多 **3 个待回覆**问题。
