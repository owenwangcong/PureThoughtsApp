...
> 版本:v0.5.19(学修问答角色语义修订,详见 §16) · 来源:`initial.md` + 十二轮需求澄清
...
- **消息角色(v0.5.19 定案,源于真机测试缺陷)**:每条消息的角色(提问人/管理员)按**发言时的身份语境**记录,**不得从"是否线程主人"推导**——管理后台回复恒为管理员身份;App 内从管理员视图(待回覆/全部)进入会话即以管理员身份发言,从「我的提問」或回复通知进入即以提问人身份发言。界面(三端)按**角色**分侧渲染并署名(管理員/提問人),管理员自问自答时谁问谁答也一目了然。管理员在 App 端列表为三个 tab:**待回覆 / 全部 / 我的提問**。
