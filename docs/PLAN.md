...
| **P8** | 学修问答(私密提问,PRD §16,设计 `design/study-qa.md`) | ⬜ 未开始 | 0/5 | — |
...
- [ ] **P8.1** 数据层(M)— migration:`qa_threads`/`qa_messages` + RLS + `qa_thread_owner` + 触发器(待回覆上限/冗余列/通知)+ RPC(`create_qa_thread`/`mark_qa_thread_read`);`push-dispatch` renderText 增两 type;pgTAP(设计 §8.1 T-DB-01…15)。验收:`npx supabase test db` 全绿。
- [ ] **P8.2** App 用户端(L)— `features/study_qa/` 列表/会话/新提问三屏 + providers;首页宫格入口 + 未登录 `_guard`;通知中心渲染与深链;l10n 三份 ARB;大字号回归接入。验收:设计 §8.2 T-APP-01…07;analyze/test 全绿。
- [ ] **P8.3** App 管理员视图(M)— `is_app_admin` 分流:待回覆/全部 tab、显示提问人、App 内回复、删任意线程。验收:设计 §8.2 T-APP-08…10。
