...
| E15 | 佛历精选节日清单的内容方审定(初版 26 条见 `docs/design/buddhist-calendar.md` §3.3;清单是生成器配置,改后重新生成即可) | 佛教日曆(P2.9) | 无(**非阻塞**,默认清单先行) | ⬜ |
...
- [ ] **P2.9** 佛教日曆(L,2026-07-16 用户需求,PRD v0.5.15 §5.2,详细设计与 TODO 见 [`design/buddhist-calendar.md`](design/buddhist-calendar.md))— 日历叠加佛历层(逐格农历副标签 + 精选 26 条佛教纪念日 + 十斋日 + 当日佛历卡);首页顶部节日/斋日横幅(匿名可见);pg_cron 每日全员通知(节日/十斋日当天 + ★ 重大提前一天预告,幂等)+ 设置页两类提醒开关;数据由 `tools/almanac/` 生成器(6tail lunar Dart 库)预生成 **2026–2075** 逐日 JSON 打进 App 资产(**完全离线**)+ 服务端 `almanac_days` 表同源产出。4 个待拍板问题(数据方案/节日范围/通知策略/用户时区)见设计 §0,未答复按推荐实现。子任务:
  - [ ] **P2.9-1** 数据生成器 `tools/almanac/` + 51 个年度 JSON 资产 + almanac 数据 migration(设计 §8-B)
  - [ ] **P2.9-2** 服务端:almanac_days/app_settings 表 + RLS + `generate_almanac_notifications()` + pg_cron + pgTAP(设计 §8-C)
  - [ ] **P2.9-3** 客户端:core/almanac 模块 + 日历格子/佛历卡 + 首页横幅 + 通知渲染过滤/设置开关 + l10n(设计 §8-D)
- [ ] **P2.10** 活动显式时区(M,2026-07-16 用户需求,PRD v0.5.15 §5)— `events.timezone`(IANA)+ 编辑器时区选择器(常用清单+全 IANA 搜索);默认值 `app_settings.default_event_timezone` 管理员可配(设置页管理员区);`expandOccurrences` 改活动时区日历算术展开(**修复每周活动跨 DST 漂移 1 小时**,存量默认 Asia/Shanghai 无回归);详情页「活動當地時間」加注;DST 单测(设计 §8-E)。
