...
- [ ] **P2.8** 正念提醒(OS 级定时,M,2026-07-14 用户需求 → PRD §9.3 详细设计)— 用户设周几×时间窗×间隔,App 关闭/息屏也能定时响磬(**本地通知,非推送**)。**设计已定稿**:[`design/mindfulness-reminders.md`](design/mindfulness-reminders.md);已定 3 决策(尊重系统静音 / 通知横幅+磬声 / 本次只出设计)。核心:把"周几×窗×间隔"展开为每周重复通知槽(安卓无限 / iOS ≤64 永久生效,超限降级);新增 `flutter_local_notifications`+`timezone`(与 P2.1 本地通知基建共享);大陆安卓需厂商自启动/电池引导(最大风险)。**待批准实现**;开放问题见设计 §15。建议与 P2.1 本地通知一起做,分安卓→iOS 两阶段。
