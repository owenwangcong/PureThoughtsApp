...
| **P1** | MVP:核心闭环(群 + 报数 + 统计)+ 上架合规 | 🔄 进行中 | 10/12(P1.1 部分完成,余 P1.10) | — |
| **P2** | 通知 / 活动 / 日历 + 发愿 + 工具 | 🔄 进行中 | 4/7(P2.1/2.2 ⛔ 待 E3/E4/E5;P2.7 部分依赖 E6) | — |
...
- [x] **P3.1** 视频(M)— 直播页(首页共修组/匿名顶栏入口):**YouTube 开播自动检测**(`live-probe` Edge Function 探测频道 /live,无需 API key;开播→`live_streams`(migration 0007)+全员通知,按场次去重;打开直播页触发探测,pg_cron 服务端轮询待 P0.2 生产部署);App 内播放器(youtube_player_iframe,直播+回看共用);Webex 固定房间一键加入(无公开状态 API,以日历预告为准,即 PRD 预判的跳转方案);回看列表(media_items 驱动);日历建活动默认预填两个频道;通知中心开播通知点击直达直播页。验收 ✅ 2026-07-08:**以真实直播端到端验证**(探测→落库→通知),pgTAP 47/47。
