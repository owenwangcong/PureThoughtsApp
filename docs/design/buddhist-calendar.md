...
> 状态:**已按推荐方案实现,待用户拍板 Q1–Q4 与真机验证(2026-07-16)** · 分支 `feature/p2-9-buddhist-calendar`
>
> **实现偏差记录**:
...
| Q2 | 节日收录范围 | A. 精选 24 条主要佛菩萨纪念日 + 十斋日(清单见 §3.3,生成器配置可改)<br>B. A + 春节/清明/中秋等传统节日(次要样式、不通知)<br>C. lunar 佛历全量 50+ 条(含冷门与"犯者夺纪"类宜忌) | **A**。C 信息噪音大且宜忌文风与本 App 不合;B 可日后加一行配置就支持 | 改清单=改 `tools/almanac/config` 后重新生成+发版;结构不变 |
...
### 3.3 精选节日清单(Q2-A 初版 24 条;生成器配置 `tools/almanac/festivals.cjs`,内容方可审改)
...
### A. 定案与文档
- [x] A1 设计文档(本文)
- [ ] A2 ⏳ **用户拍板 Q1–Q4**(已按推荐实现;若改选,更新本文 §0 与受影响小节)
- [x] A3 PRD 升 v0.5.15:§5.2 佛教日曆小节 + 跨时区补充;§12.2 新表/新列;§12.3 RLS;§12.4 cron 任务
- [x] A4 PLAN 增补 P2.9 / P2.10 任务与外部依赖 E15(节日清单内容方审定,非阻塞)

### B. 数据生成器(P2.9-1)✅ 2026-07-16
- [x] B1 `tools/almanac/` 生成器骨架(**Node + lunar-javascript**,见头部偏差记录;README 再生成流程)
- [x] B2 精选节日配置 `festivals.cjs`(§3.3 清单 24 条,简繁+短名+★)
- [x] B3 生成逻辑:逐日农历、十斋日、节日匹配(两遍扫描;九月三十小月回退、闰月不过节,已在 2026/2027 数据实证)
- [x] B4 校验断言(春节 2026/2027、佛诞 2026-05-24 锚点;年天数;节日出现次数)+ sanity 报告
- [x] B5 产出并提交:`app/assets/almanac/*.json`(51 个文件,416KB)+ `20260716000013_almanac_data.sql`(6546 行)
- [x] B6 pubspec assets 注册 `assets/almanac/`

### C. 服务端(P2.9-2 / P2.10-1)✅ 2026-07-16
- [x] C1 migration `20260716000012_almanac_tz.sql`:almanac_days + app_settings(seed 默认时区 Asia/Shanghai)+ events.timezone + RLS(并把 timezone 纳入活动变更通知比较元组)
- [x] C2 `generate_almanac_notifications()`(幂等,kind=festival/zhai/festival_eve)+ pg_cron `almanac-daily`(5 16 * * * = UTC+8 00:05,本地栈已验证注册)
- [x] C3 pgTAP `almanac.test.sql` 10 项(RLS/锚点/幂等),`supabase test db` 66/66 全绿
- [ ] C4 ⏳ 本地栈通知中心可见性走查(手动 `select generate_almanac_notifications()` → App 通知中心显示)——函数产出已由 pgTAP 断言,App 侧待真机随 F3 一并验

### D. 客户端佛历(P2.9-3)✅ 2026-07-16
- [x] D1 `core/almanac/`(almanac.dart + lunar_format.dart):模型、每年懒加载 provider、农历文案简繁渲染;单测 lunar_format 5 项 + almanac 9 项
- [x] D2 日历格子:农历副标签 + 节日短名(金色)+ 十斋日角点;FittedBox 整体收缩 + 格内 1.3× 缩放上限,2.0 字号走查通过
- [x] D3 当日佛历卡(特殊日 Card 强调,平日一行灰字农历)
- [x] D4 首页横幅 `_AlmanacBanner`(匿名可见、平日不占位、点击进日历)+ `almanacBannerLines` 纯函数 3 项单测
- [x] D5 设置页「佛曆提醒」两开关 + 通知中心 almanac 渲染(按语言取简繁 payload)与过滤(`visibleNotificationsProvider`,红点同步)+ 过滤 3 项单测
- [x] D6 l10n 三份 ARB 各新增 15 键

### E. 活动时区(P2.10-2)✅ 2026-07-16
- [x] E1 活动编辑器时区选择器(常用 14 城 + 全 IANA 搜索;新建默认读 app_settings;编辑回显原时区墙钟)
- [x] E2 设置页管理员区「預設活動時區」(upsert app_settings)
- [x] E3 `expandOccurrences` 活动时区日历算术展开;单测 5 项:春/秋 DST 墙钟不漂移、上海严格 +168h、缺列回退、dateKey=活动时区日期
- [x] E4 活动详情页「活動當地時間」加注(墙钟与设备显示不同才出现)
- [x] E5 tz 初始化:`core/timezones.dart#ensureTimeZonesInitialized()` 幂等共用(main 的 ReminderScheduler 与展开/编辑器各自兜底)

### F. 收尾
- [x] F1 `flutter analyze` 0 issue + `flutter test` 134/134 + pgTAP 66/66
- [x] F2 大字号走查:活動日曆佛历格子进 `layout_walkthrough_test`(简繁 × 2.0;曾捕获 1px 溢出并修复)
- [ ] F3 ⏳ 真机走查:横幅显示、日历农历/节日、管理员建跨时区活动、通知中心收到节日通知(本地栈手动触发)
- [x] F4 PLAN 勾选与进度计数更新;本文档记录偏差(见头部)
