...
> 状态:**已定案(2026-07-18,用户选定「默认方案」= 核心 + 已排程列表 + 撤回)** · PRD v0.5.16 · PLAN P2.11
...
- [ ] T2 migration 0015:两个 RPC + pgTAP(管理员可发/普通用户拒;取消仅限 general;定时行 sent_at 为空)
- [ ] T3 发布界面(表单/定时/预览确认/排程列表/撤回)+ 路由 + l10n
- [ ] T4 首页管理分组 + 檢舉處理迁入(设置页移除原入口)
- [ ] T5 通知中心过滤 scheduled 未到点行(修复提前可见)
- [ ] T6 质量门(analyze/test/pgTAP)+ 真机走查(发即时→秒级收推送;发定时→列表可见可取消;撤回→中心消失)
