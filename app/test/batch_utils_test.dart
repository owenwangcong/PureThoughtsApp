...
void main() {
  ...
  group('重複上次 latestBatch', () {
    ...
    test('同日同功课多条取最新一条', () {
      ...
      expect(latestBatch(logs), {'jing': 3});
    });

    test('无记录返回空', () {
      expect(latestBatch([]), isEmpty);
    });
  });
  ...
  group('常用功课 frequentTypeIds', () {
    test('去重保持最近优先,限制数量', () {
      ...
      expect(frequentTypeIds(logs), ['a', 'b', 'c']);
      expect(frequentTypeIds(logs, limit: 2), ['a', 'b']);
    });
  });
}
